#include <bits/stdc++.h>
using namespace std;
map<string, pair<int, string>> OPTAB; // instruction addCount OpNumber
...
string inttohex(int x, int b)
{
    ...
    sstream << setfill('0') << setw(b) << hex << (int)x; // 以16進位輸入
    s = sstream.str();
    sstream.clear();
...
string normalInstruction(int i)
{
...
string extendMode(int bb)
{
    string z = prog[bb].second[2], te = prog[bb].second[1], TA = "", objcode;
    int no = 0;
...
void ObjectFile()
{
    freopen("objectOutput.txt", "w", stdout);
    int sz = objcode.size();
    cout << "H^" << programName;
    for (int i = 0; i < 6 - programName.size(); i++)
        cout << " ";
    cout << "^" << inttohex(objcode[0].first, 6) << "^" << inttohex(LOCCTR, 6) << endl;

    for (int i = 0; i < objcode.size(); i += 5)
    {
        long long sum = 0;
        for (int j = i; j < i + min(sz - i, 5); j++)
        {
            sum += objcode[j].second.size() / 2;
        }
        cout << "T^" << inttohex(objcode[i].first, 6) << "^" << inttohex(sum, 2);

        for (int j = i; j < i + min(sz - i, 5); j++)
        {
            cout << "^" << objcode[j].second;
        }
        cout << endl;
    }
    for (int i = 0; i < modify.size(); i++)
        cout << "M^" << inttohex(modify[i] + 1, 6) << "^05" << endl;
    cout << "E^" << inttohex(objcode[0].first, 6);
    fclose(stdout);
}

void listFile()
{
    freopen("listFile", "w", stdout);
    for (int i = 0; i < prog.size(); i++)
    {
        if (prog[i].second[0] == "*")
        {
            if (prog[i].second[2].length() == 8)
                ...
                cout << inttohex(prog[i].first, 4) << "\t\t" << prog[i].second[1] << "\t"
                     << " "
        ...
        if (i == 0)
            ...
            objcode.push_back({prog[i].first, prog[i].second[3]});
    }
}
...
int main()
{
    ...
    {
        cin >> x >> y;
        REGS[x] = y; // maps every register name with prog's number
    }
    ...
    while (getline(input, l))
    {
        ...
        {
            if (status && OPTAB.find(y) == OPTAB.end() && y != "END" && y != "BASE" && y != "NO BASE")
            {
                clog << "invalid input please check your input format or instruction";
                return 0;
            }
        }
    ...
    for (int i = 0; i < prog.size(); i++)
    {
        ...
        {
            if (prog[j].first != prog[i].first) // 跑到location counter不同
                break;
        }
        pc = prog[j].first; // pc就在location counter第一次不同的地方
        string s2 = prog[i].second[1];
        int format = OPTAB[s2].first;
        if (format == 1)
        {
            s = OPTAB[prog[i].second[1]].second; // opNumber
        }
        else if (format == 2)
        {
            s = compareRegister(i);
        ...
        /*clog << prog[i].first << " " << prog[i].second[0] << " " << prog[i].second[1] << " " << prog[i].second[2] << " " << prog[i].second[3] << endl;*/
    }

    listFile();
    ObjectFile();

    clog << " assemble successfully please check ObjectOuput File and list File" << endl;
}
